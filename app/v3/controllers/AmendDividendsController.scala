...
import v3.services.AmendDividendsService

import scala.concurrent.{ExecutionContext, Future}


class AmendDividendsController @Inject()(
                                          val authService: EnrolmentsAuthService,
                                          val lookupService: MtdIdLookupService,
                                          amendDividendsService: AmendDividendsService,
  ...
  def amend(nino: String, taxYear: String): Action[JsValue] = authorisedAction(nino).async(parse.json) { implicit request =>
    ...
    DslDefinition(
      endpointName = "AmendDividendsIncome",
      desEndpoint = s"/income-tax/nino/$nino/income-source/dividends/annual/$taxYear"
      // httpReads
    )

//    amendDividendsService.amend()





    Future(Ok("IN PROGRESS"))
  }

}


case class DslDefinition(
                          endpointName: String,
                          desEndpoint: String
                        )
