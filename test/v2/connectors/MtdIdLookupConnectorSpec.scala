...
class MtdIdLookupConnectorSpec extends ConnectorSpec {
  ...
  class Test extends MockHttpClient with MockAppConfig {
    ...
    MockedAppConfig.mtdIdBaseUrl returns url
  }

  lazy val url = "http://test-mtdIdBaseUrl"
  ...
  "getMtdId" should {
    "return an MtdId" when {
      "the http client returns a mtd id" in new Test {
        MockedHttpClient.get[MtdIdLookupOutcome](s"$url/mtd-identifier-lookup/nino/$nino", dummyDesHeaderCarrierConfig)
